...
Usage:
  python scripts/regex_replace.py path/to/file.txt
  python scripts/regex_replace.py --in-place path/to/file.txt
"""
...
PATTERN_REPLACEMENTS: Iterable[Tuple[re.Pattern[str], str]] = (
    (re.compile(r"\bDiab(solut)?\b", re.ASCII), "AcmeCorp"),
    (re.compile(r"\bMarathonOil\b", re.ASCII), "MegaCorp"),
    (re.compile(r"\bmarathonoil\b", re.ASCII), "megacorp"),
)
